...
protected:
  ...
  constexpr data_t &_query(const pointer &now, index_t &&l, index_t &&r, index_t &&pos) const {
    ...
    if (pos <= mid)
      ...
      return _query(now->r, mid + 1, std::move(r), std::move(pos));
  }
...
public:
  ...
  // for debug
  inline constexpr size_t memory_used() const { return sizeof(node_t) * nodes.capacity() + sizeof(pointer) * roots.capacity(); }
