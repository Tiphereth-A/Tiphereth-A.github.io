...
M operator+(M a, M b) { return M(a.x + b.x); }
M operator-(M a, M b) { return M(a.x - b.x); }
M operator-(M a) { return M(-a.x); }
M operator*(M a, M b) { return M(a.x * b.x); }

M arithSum(Z a, Z b) { return (b * (b + 1) - a * (a - 1)) / 2; }
...
bool bw(int a, int x, int b) { return a <= x && x <= b; }
...
int main() {
  ...
  for (int i = 0; i < q; ++i) {
    ...
    for (int i = 1; i < (int)events.size(); ++i) {
      ...
      if (k >= 4) {
        M v[4];
        for (int j = 0; j < 4; ++j) { v[j] = layerSum(a + j, x1, y1, x2, y2); }
        ...
        for (int l = a; l < b; ++l) { ret = ret + layerSum(l, x1, y1, x2, y2); }
      }
    }
