...
- $$\kappa(t)={|\textbf{r}'(t)\wedge\textbf{r}''(t)|\over|\textbf{r}'(t)|^3}={x'y''-x''y'\over(x'^2+y'^2)^{3\over 2}}$$
...
- **常曲率曲线** 我们有如下定理
  ...
  **<a id="th-2-1">定理 - 2-1</a>** 对于常曲率平面曲线 $\textbf{r}(s)$
  ...
  - 若 $\kappa(s)\equiv k\ne 0$, 则 $\textbf{r}(s)$ 是圆, 且圆的半径为 ${1\over |k|}$
  ...
  $$\begin{aligned}
    ...
    \xLeftrightarrow{\textbf{r}(s)+{1\over k}\textbf{n}(s)=:\textbf{p}(s)}&\textbf{p}'(s)=0\\
    \iff&\begin{cases}
        \textbf{p}(s)\equiv\textbf{p}_0\\
        |\textbf{r}(s)-\textbf{p}_0|\equiv {1\over k}
    \end{cases}
  \end{aligned}$$
...
- $$\kappa(t)={\sqrt{(y'z''-y''z')^2+(z'x''-x'z'')^2+(x'y''-y'x'')^2}\over(x'^2+y'^2+z'^2)^{3\over2}}$$
- $$\tau(t)={x'''(y'z''-y''z')+y'''(z'x''-x'z'')+z'''(x'y''-y'x'')\over(y'z''-y''z')^2+(z'x''-x'z'')^2+(x'y''-y'x'')^2}$$
...
- 若 $\kappa(s)\equiv 0$, 则 $\textbf{r}(s)$ 是直线
- 若 $\kappa(s)\equiv k\ne 0$
  - 若 $\tau(s)\equiv 0$, 则 $\textbf{r}(s)$ 是圆, 且圆的半径为 ${1\over |k|}$
...
- $$\tilde\textbf{n}(s)={1\over\tilde\kappa(s)}{\mathrm{d}\tilde\textbf{t}\over\mathrm{d}s}(s)=\textbf{n}(s)T$$
