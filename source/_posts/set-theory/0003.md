...
> $$\left\{x\in\R:\lim_{y\to x}g(y)={\pi\over2}\right\}$$
...
$$\forall x\in(x_1,x_2)\subseteq(a,b),~f(x)\leqslant{(x_2-x)f(x_1)-(x-x_1)f(x_2)\over x_2-x_1}$$

即

$${f(x)-f(x_1)\over x-x_1}\leqslant{f(x_2)-f(x)\over x_2-x}$$

又

$$\forall x'\in(x,x_2),~{f(x')-f(x)\over x'-x}\leqslant{f(x_2)-f(x)\over x_2-x}$$

故

$$\lim_{x'\to x^+}{f(x')-f(x)\over x'-x}=f'_+(x)<+\infty$$
...
取 $E_n=\{x\in E:x\geqslant{1\over n}\},~n=2,3,...$, 则 $E=\bigcup_{n=2}^{\infty}E_n$

显然 $E_n$ 为可数集, 否则可以取出数列 $\{a_n\}$ 使得级数 $\sum a_n\geqslant\sum{1\over n}=+\infty$
